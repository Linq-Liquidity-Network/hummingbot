...
2. From hummingbot/connector/exchange, copy the contents of bitbay:

    cp ./bitbay/* your_connector_name

3. Rename the files in the folder so that the preface bitbay is replaced by your 
...
4. Perform the following series of replacements using search and replace (restricted to your new connector's
   directory):
   i) Bitbay -> YourConnectorName (note that your_connector_name in step 1 is converted to precisely
      YourConnectorName)
   ii) BITBAY -> YOUR_CONNECTOR_NAME (note capitalization)
   iii) bitbay.net -> your_connector_name (this is mostly done to distinguish the
