...
class BitbayAPITokenConfigurationDataSource():
    """ Gets the token configuration on creation.

        Use BitbayAPITokenConfigurationDataSource.create() to create.
    """
    ...
    async def _configure(self):
        async with aiohttp.ClientSession() as client:
            response: aiohttp.ClientResponse = await client.get(
                f"https://api.bitbay.io{TOKEN_CONFIGURATIONS_URL}"
            )

            if response.status >= 300:
                raise IOError(f"Error fetching active bitbay token configurations. HTTP status is {response.status}.")
