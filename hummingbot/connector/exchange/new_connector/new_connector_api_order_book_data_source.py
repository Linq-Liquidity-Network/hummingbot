...
class NewConnectorAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_snapshot(self, client: aiohttp.ClientSession, trading_pair: str, level: int = 0) -> Dict[str, any]:
        async with client.get(f"https://api.new_connector.io{SNAPSHOT_URL}&level={level}".replace(":trading_pair", trading_pair)) as response:
            ...
            if response.status != 200:
                raise IOError(
                    f"Error fetching new_connector market snapshot for {trading_pair}. " f"HTTP status is {response.status}."
                )
    ...
    async def fetch_trading_pairs() -> List[str]:
        try:
            async with aiohttp.ClientSession() as client:
                async with client.get(f"https://api.new_connector.io{MARKETS_URL}", timeout=5) as response:
                    if response.status == 200:
            ...
            # Do nothing if the request fails -- there will be no autocomplete for new_connector trading pairs
            pass
