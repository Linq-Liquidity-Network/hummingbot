...
cdef class NewConnectorExchange(ExchangeBase):
    ...
    def __init__(self,
                 new_connector_accountid: int,
                 new_connector_exchangeid: int,
                 new_connector_private_key: str,
                 new_connector_api_key: str,
        ...
        self._new_connector_accountid = int(new_connector_accountid)
        self._new_connector_exchangeid = int(new_connector_exchangeid)
        self._new_connector_private_key = new_connector_private_key
    ...
    def limit_orders(self) -> List[LimitOrder]:
        ...
        for in_flight_order in self._in_flight_orders.values():
            ...
            if new_connector_flight_order.order_type is OrderType.LIMIT:
                retval.append(new_connector_flight_order.to_limit_order())
        return retval
    ...
    async def _get_next_order_id(self, token, force_sync = False):
        async with self._order_id_lock:
            ...
            if force_sync or self._next_order_id.get(token) is None:
                try:
                    response = await self.api_request("GET", NEXT_ORDER_ID, params={"accountId": self._new_connector_accountid, "tokenSId": token})
                    next_id = response["data"]
                    self._next_order_id[token] = next_id
                except Exception as e:
                    self.logger().info(str(e))
                    self.logger().info("Error getting the next order id from New_Connector")
            else:
    ...
    async def place_order(self,
        ...
        order = {
            "exchangeId": self._new_connector_exchangeid,
            "orderId": order_id,
            "accountId": self._new_connector_accountid,
            "allOrNone": "false",
    ...
    async def execute_order(self, order_side, client_order_id, trading_pair, amount, order_type, price):
        ...
        try:
            ...
            in_flight_order.update_exchange_order_id(new_connector_order_hash)
            ...
            self.logger().warning(f"Error submitting {order_side.name} {order_type.name} order to New_Connector for "
    ...
    cdef object c_get_fee(self,
        ...
        return estimate_fee("new_connector", is_maker)
    ...
    async def _polling_update(self):
        while True:
            try:
                ...
                self.logger().warning("Failed to fetch updates on New Connector. Check network connection.")
                self.logger().info(e)
    ...
    async def _update_order_status(self):
        ...
        for client_order_id, tracked_order in tracked_orders.iteritems():
            new_connector_order_id = tracked_order.exchange_order_id
            if new_connector_order_id is None:
                ...
                if tracked_order.created_at < (int(time.time()) - UNRECOGNIZED_ORDER_DEBOUCE):
                    # this order should have a new_connector_order_id at this point. If it doesn't, we should cancel it
            ...
            try:
                new_connector_order_request = await self.api_request("GET",
                                                                GET_ORDER_ROUTE,
                                                                params={
                                                                    "accountId": self._new_connector_accountid,
                                                                    "orderHash": tracked_order.exchange_order_id
                                                                })
                data = new_connector_order_request["data"]
            except Exception:
                self.logger().warning(f"Failed to fetch tracked New Connector order " \
                                      f"{client_order_id }({tracked_order.exchange_order_id}) from api (code: {new_connector_order_request['resultInfo']['code']})")
                ...
                if new_connector_order_request['resultInfo']['code'] == 107003:
                    if tracked_order.created_at < (int(time.time()) - UNRECOGNIZED_ORDER_DEBOUCE):
    ...
    async def api_request(self,
        ...
        headers.update(self._new_connector_auth.generate_auth_dict())
        ...
        async with self._shared_client.request(http_method, url=full_url,
            ...
            if response.status != 200:
                self.logger().info(f"Issue with New Connector API {http_method} to {url}, response: ")
