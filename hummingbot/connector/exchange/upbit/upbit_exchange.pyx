...
from hummingbot.connector.exchange.upbit.upbit_in_flight_order cimport UpbitInFlightOrder
...
cdef class UpbitExchangeTransactionTracker(TransactionTracker):
    cdef:
        UpbitExchange _owner

    def __init__(self, owner: UpbitExchange):
        super().__init__()
        self._owner = owner
...
cdef class UpbitExchange(ExchangeBase):
    ...
    def __init__(self,
        ...
        self._user_stream_tracker = UpbitUserStreamTracker(
            orderbook_tracker_data_source=self._order_book_tracker.data_source,
            upbit_auth=self._upbit_auth
        )
        self._user_stream_event_listener_task = None
        self._user_stream_tracker_task = None
        self._tx_tracker = UpbitExchangeTransactionTracker(self)
    ...
    def limit_orders(self) -> List[LimitOrder]:
        cdef:
            list retval = []
            UpbitInFlightOrder upbit_flight_order

        for in_flight_order in self._in_flight_orders.values():
            upbit_flight_order = in_flight_order
            if upbit_flight_order.order_type is OrderType.LIMIT:
                retval.append(upbit_flight_order.to_limit_order())
        return retval

    async def get_active_exchange_markets(self) -> pd.DataFrame:
        return await UpbitAPIOrderBookDataSource.get_active_exchange_markets()
    ...
    async def execute_order(self, order_side, client_order_id, trading_pair, amount, order_type, price):
        ...
        try:
            ...
            self.logger().warning(f"Error submitting {order_side.name} {order_type.name} order to upbit for "
    ...
    cdef object c_get_fee(self,
        ...
        return estimate_fee("upbit", is_maker)
    ...
    def restore_tracking_states(self, saved_states: Dict[str, any]):
        for order_id, in_flight_repr in saved_states.iteritems():
            ...
            self._in_flight_orders[order_id] = UpbitInFlightOrder.from_json(self, in_flight_json)
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg="Could not fetch user events from upbit. Check API key and network connection."
                )
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                if topic == 'account':
                    ...
                    tracked_order: UpbitInFlightOrder = self._in_flight_orders.get(client_order_id)
    ...
    async def _polling_update(self):
        while True:
            try:
                ...
                self.logger().warning("Failed to fetch updates on upbit. Check network connection.")
                self.logger().info(e)
    ...
    async def _update_order_status(self):
        ...
        for client_order_id, tracked_order in tracked_orders.iteritems():
            upbit_order_id = tracked_order.exchange_order_id
            if upbit_order_id is None:
                ...
                if tracked_order.created_at < (int(time.time()) - UNRECOGNIZED_ORDER_DEBOUCE):
                    # this order should have a upbit_order_id at this point. If it doesn't, we should cancel it
